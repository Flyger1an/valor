...
import { augmentedDickeyFullerTest, clamp, round, zScore } from "@/lib/utils/math";
...
function btcEthRatioSignals(
  ...
  const direction = meanReversionDirection(score, 1.1, stationarity.isStationary);
  ...
  return [
    buildSignal({
      ...
      timestamp,
      eligibleForPaperTrading:
        stationarity.isStationary && Math.abs(score) > 1.1,
      spreadStationary: stationarity.isStationary,
      adfTestStatistic: stationarity.testStatistic,
      explanation: `BTC/ETH ratio z-score is ${round(score, 2)}. ${
        stationarity.note
...
function pairSpreadSignals(
  ...
  const direction = meanReversionDirection(score, 1.25, stationarity.isStationary);
  ...
  return [
    buildSignal({
      ...
      timestamp,
      eligibleForPaperTrading:
        stationarity.isStationary && Math.abs(score) > 1.25,
      spreadStationary: stationarity.isStationary,
      adfTestStatistic: stationarity.testStatistic,
      explanation: `${pair} normalized spread z-score is ${round(score, 2)}. ${
        stationarity.note
...
function assessSpreadStationarity(series: number[]) {
  const result = augmentedDickeyFullerTest(series, STATIONARITY_SIGNIFICANCE);
  const isStationary = result.isStationary;
  const note = isStationary
    ? `ADF(5%) t=${round(result.testStatistic, 2)} rejects unit root (${result.confidence}).`
    : series.length < 12
      ? `Only ${series.length} history points are available; need 12 for ADF, so this stays watch-only.`
      : `ADF(5%) t=${round(result.testStatistic, 2)} does not reject unit root; mean-reversion thesis is watch-only.`;
...
function meanReversionDirection(
  ...
  if (!isStationary) return "watch_only";

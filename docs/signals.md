...
- `zscore`, `spreadValue`, and `expectedConvergenceHours`
...
Edge estimates are net of conservative execution-cost assumptions from `src/lib/signals/costs.ts`. Mean-reversion signal histories can be reconstructed from live exchange candles when public market fetch is enabled, with fixture lineage recorded when live history is unavailable.
